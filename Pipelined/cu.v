    ...
    always @(*) begin
        ...
        case(opcode)
            ...
            7'b1101111: begin
                ...
                ALUSrc   = 1'b1; //prevent X on SrcBE (J-type has no rs2)
                ALUOp    = 2'b00; //add (ALU result unused; target = PCE + ImmExtE)
            end
            //jalr: indirect jump, write PC+4 to rd, target = rs1 + imm
            7'b1100111: begin
                RegWrite = 1'b1; //write return address (PC+4) to rd
                ALUSrc   = 1'b1; //use I-type immediate so ALU computes rs1 + imm = jump target
                ALUOp    = 2'b00; //add
            end
        endcase
    end
